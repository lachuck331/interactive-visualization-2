...
var app = (function () {
    ...
    // (96:8) {#each allDepartments as dept}
    function create_each_block(ctx) {
    	let button;
    	let t0_value = /*dept*/ ctx[9] + "";
    	let t0;
    	let t1;
    	let button_data_dept_value;
    	let mounted;
    	let dispose;

    	function click_handler() {
    		return /*click_handler*/ ctx[4](/*dept*/ ctx[9]);
    	}
    	...
    	const block = {
    		c: function create() {
    			...
    			t0 = text$1(t0_value);
    			t1 = space();
    			attr_dev(button, "data-dept", button_data_dept_value = /*dept*/ ctx[9]);

    			set_style(button, "color", /*selectedDepartments*/ ctx[1].has(/*dept*/ ctx[9])
    			? /*colorScale*/ ctx[2](/*dept*/ ctx[9])
    			: 'black');

    			set_style(button, "border-color", /*selectedDepartments*/ ctx[1].has(/*dept*/ ctx[9])
    			? /*colorScale*/ ctx[2](/*dept*/ ctx[9])
    			: '#ccc');

    			attr_dev(button, "class", "svelte-gq1i96");
    			add_location(button, file, 96, 8, 3788);
    		},
    		...
    		p: function update(new_ctx, dirty) {
    			ctx = new_ctx;
    			if (dirty & /*allDepartments*/ 1 && t0_value !== (t0_value = /*dept*/ ctx[9] + "")) set_data_dev(t0, t0_value);

    			if (dirty & /*allDepartments*/ 1 && button_data_dept_value !== (button_data_dept_value = /*dept*/ ctx[9])) {
    				attr_dev(button, "data-dept", button_data_dept_value);
    			}

    			if (dirty & /*allDepartments*/ 1) {
    				set_style(button, "color", /*selectedDepartments*/ ctx[1].has(/*dept*/ ctx[9])
    				? /*colorScale*/ ctx[2](/*dept*/ ctx[9])
    				: 'black');
    			}
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(96:8) {#each allDepartments as dept}",
    		ctx
    	});
    ...
    function create_fragment(ctx) {
    	...
    	let t4;
    	let svg;
    	let each_value = /*allDepartments*/ ctx[0];
    	...
    	const block = {
    		c: function create() {
    			main = element("main");
    			h1 = element("h1");
    			h1.textContent = "Study Hours Analysis";
    			t1 = space();
    			i = element("i");
    			i.textContent = "Welcome to our webpage dedicated to studying habits at UC San Diego. Did you know that, on average, students at UCSD dedicate approximately 5.5 hours per week to studying for each class they take?\r\n\r\n\tCurious about how your own major compares? Check out our interactive analysis where you can explore the study habits across different departments. Click on the buttons representing various majors to see how their study hours stack up against each other, and the campus average.";
    			t3 = space();
    			div = element("div");
    			...
    			t4 = space();
    			svg = svg_element("svg");
    			add_location(h1, file, 89, 4, 3210);
    			add_location(i, file, 90, 1, 3242);
    			attr_dev(div, "class", "svelte-gq1i96");
    			add_location(div, file, 94, 1, 3733);
    			attr_dev(svg, "id", "chart");
    			attr_dev(svg, "class", "svelte-gq1i96");
    			add_location(svg, file, 101, 4, 4081);
    			attr_dev(main, "class", "svelte-gq1i96");
    			add_location(main, file, 88, 0, 3198);
    		},
    		...
    		p: function update(ctx, [dirty]) {
    			if (dirty & /*allDepartments, selectedDepartments, colorScale, toggleDepartment*/ 15) {
    				each_value = /*allDepartments*/ ctx[0];
    				validate_each_argument(each_value);
    				let i;
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	onMount(() => {
    		csv('https://lachuck331.github.io/interactive-visualization-2/capes_data.csv').then(loadedData => {
    			data = loadedData;
    			processData();
    			drawGraph();
    		}).catch(error => {
    	...
    	function processData() {
    		...
    		const groupByYear = rollup(data, v => mean(v, d => d['Study Hours per Week']), d => d.Year);
    		summaryByYear = Array.from(groupByYear, ([Year, AverageStudyHours]) => ({ Year, AverageStudyHours }));
    		$$invalidate(0, allDepartments = Array.from(new Set(data.map(d => d.Department))));
    	}
    	...
    	function drawGraph() {
    		...
    		const maxStudyHours = max$3([
    			...summaryByYear.map(d => d.AverageStudyHours),
    			...Array.from(selectedDepartments).flatMap(dept => data.filter(d => d.Department === dept).map(d => d['Study Hours per Week']))
    		]);

    		const margin = { top: 20, right: 30, bottom: 30, left: 50 };
    		const width = 800 - margin.left - margin.right;
    		const height = 400 - margin.top - margin.bottom;
    		const xScale = linear().domain(extent$1(summaryByYear, d => +d.Year)).range([0, width]);
    		const yScale = linear().domain([0, maxStudyHours]).range([height, 0]);
    		const g = svg.append('g').attr('transform', `translate(${margin.left},${margin.top})`);
    		g.append('g').attr('transform', `translate(0,${height})`).call(axisBottom(xScale).tickFormat(format("20")));
    		g.append('g').call(axisLeft(yScale));
    		const line$1 = line().x(d => xScale(+d.Year)).y(d => yScale(d.AverageStudyHours));
    		g.append('path').datum(summaryByYear).attr('fill', 'none').attr('stroke', 'steelblue').attr('stroke-width', 2).attr('d', line$1);

    		selectedDepartments.forEach(dept => {
    			const deptData = data.filter(d => d.Department === dept);
    			const deptSummary = rollup(deptData, v => mean(v, d => d['Study Hours per Week']), d => d.Year);
    			const deptSummaryArray = Array.from(deptSummary, ([Year, AverageStudyHours]) => ({ Year, AverageStudyHours }));
    			g.append('path').datum(deptSummaryArray).attr('fill', 'none').attr('stroke', colorScale(dept)).attr('stroke-width', 2).attr('d', line$1);
    		});
    	}
    	...
    	const click_handler = dept => toggleDepartment(dept);
    	...
    	$$self.$inject_state = $$props => {
    		...
    		if ('summaryByYear' in $$props) summaryByYear = $$props.summaryByYear;
    		if ('allDepartments' in $$props) $$invalidate(0, allDepartments = $$props.allDepartments);
    		if ('selectedDepartments' in $$props) $$invalidate(1, selectedDepartments = $$props.selectedDepartments);
